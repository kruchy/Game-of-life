...
-compile(export_all).% Author : Krzysztof Misiak

% Initiates view board with given size.
initBoard(Width, Height) ->
    array:new([{size, Width},
	       {default, array:new([{size, Height}, {default, o}])}]).
...
loop(Board, Width, Height, Received)
    when Width * Height =:= Received ->
    drawBoard(Board), loop(Board, Width, Height, 0);
loop(Board, Width, Height, Received) ->
    receive
      {update, X, Y, State} ->
	  case State of
	    alive ->
		NewBoard = set(X, Y, x, Board),
		loop(NewBoard, Width, Height, Received + 1);
	    dead ->
		NewBoard = set(X, Y, o, Board),
		loop(NewBoard, Width, Height, Received + 1)
	  end;
      {draw} ->
	  drawBoard(Board), loop(Board, Width, Height, Received)
    end.
