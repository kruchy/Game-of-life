...
lightWeight() -> [{6,6},{7,6},{4,7},{5,7},{7,7},{8,7},{4,8},{5,8},{6,8},{7,8},{5,9},{6,9}].
...
cell(X,Y,State,Neighbors,Living,Received) ->
	receive
		...
		{set,NewS} -> com ! {update,X,Y,NewS} ,cell(X,Y,NewS,Neighbors,Living,Received);
...
neighborsLength([[] | Rest],Length) -> neighborsLength(Rest,Length);
neighborsLength([[_|Rest]],Length) -> neighborsLength(Rest,Length + 1).
