...
start(Width, Height, _) when Width < 3, Height < 3 ->
    throw(invalid_size);
start(_, _, Alive) when length(Alive) =:= 0 ->
    throw(empty_board);
start(Width, % first dimension
      Height, % second dimension
      Alive) -> % list of alive cells
    case lists:member(view, registered()) of
      true -> ok;
      false ->
	  register(com,
		   spawn_link(view, loop,
			      [view:initBoard(Width, Height), Width, Height,
			       0]))
    end,
